...
struct Heap::Block {
  ...
  struct Header {
    ...
    explicit Header(Use_Status use_status = Free, const char *tag_value = nullptr)
  ...
  explicit Block (usize block_size)
    : size { block_size }
  {}
...
u8* alloc (Heap &heap, const usize size, const char *tag) {
  assert(size != 0);
  assert(tag != nullptr);
...
u8* realloc (Heap &heap, void * const ptr, const usize new_size, const char * const tag) {
  assert(new_size > 0);
  assert(tag != nullptr);
...
void free (Heap &heap, void * const ptr, const char * const tag) {
  assert(ptr != nullptr);
  assert(tag != nullptr);
...
const char* find_tag (Heap &heap, void *ptr) {
